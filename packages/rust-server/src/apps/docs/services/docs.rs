use crate::db::repos::docs_repo::{DocNodeRepo, DocNodeVersionRepo, UpdateDocNodeInput};
...
impl DocsService {
    ...
    pub async fn update_node_with_version(
        ...
        let word_count = content.as_ref().and_then(|opt_content| {
            opt_content.as_ref().map(Self::count_words)
        });

        let search_text = content.as_ref().and_then(|opt_content| {
            opt_content.as_ref().map(Self::extract_text)
        });
