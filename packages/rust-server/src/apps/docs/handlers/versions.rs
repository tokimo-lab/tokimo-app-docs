...
use crate::AppState;
use crate::apps::docs::models::{DocNodeVersionDetailOutput, DocNodeVersionOutput};
...
pub async fn list_versions(
    ...
    let versions = DocNodeVersionRepo::list(&state.db, parse_uuid(&id)?, &q.rel_path).await?;
...
pub async fn get_version(
    ...
    let version = DocNodeVersionRepo::get_by_id(&state.db, parse_uuid(&version_id)?)
        .await?
